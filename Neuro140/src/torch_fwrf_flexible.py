...
class Torch_fwRF_voxel_block_flexible(nn.Module):
    '''
    ...
    def __init__(self, _fmaps_fn, params, _nonlinearity=None, input_shape=(1,3,227,227), aperture=1.0):
        ...
        for k,n_pix in enumerate(self.fmaps_rez):
            # shape: [nv, h, w] for each feature map
            pf = nn.Parameter(torch.randn((nv, n_pix, n_pix), device=device) * 1e-2, requires_grad=True)
            self.pfs += [pf]
            self.register_parameter('pf%d'%k, pf)
    ...
    def forward(self, _fmaps):
        B, C, H, W = _fm.shape
        _pf_flat = _pf.view(nv, -1)                          # [nv, H*W]
        _pf_soft = torch.softmax(_pf_flat, dim=1)            # [nv, H*W]
        _fm_flat = _fm.view(B, C, -1)                        # [B, C, H*W]
        _mst = torch.einsum('vc,bcp->bvp', _pf_soft, _fm_flat) # [B, nv, C]
...
def learn_params_ridge_regression(data, voxels, _fmaps_fn, models, lambdas, aperture=1.0, _nonlinearity=None, zscore=False, sample_batch_size=100, voxel_batch_size=100, holdout_size=100, shuffle=True, add_bias=False,precomputed_fmaps=None):
    """
    ...
    with torch.no_grad():
        for m,(x,y,sigma) in enumerate(models):
            ...
            for batch_i, (rt, rl) in enumerate(iterate_range(0, nt, sample_batch_size)):
                fmaps_batch = precomputed_fmaps[batch_i] if precomputed_fmaps is not None else _fmaps_fn(_to_torch(data[rt],device=device))
                _mst = torch.cat([
                    torch.tensordot(_fm, _pf, dims=[[2,3], [0,1]])
                    for _fm, _pf in zip(fmaps_batch, _pfs)
                ], dim=1)
    ...
    ############################################################################# 
    print("voxel batch [%d:%d]" % (rv[0], rv[-1]))
    print("imp.sum() =", np.sum(imp))
    print("betas shape =", betas.shape)
    print("select shape =", betas[:,:,imp].shape)
