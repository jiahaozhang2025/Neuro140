...
def save_stuff_2(save_to_this_file, data_objects_dict):
    failed = []

    # Ensure directory exists
    os.makedirs(os.path.dirname(save_to_this_file), exist_ok=True)
    ...
    with h5py.File(save_to_this_file + '.h5py', 'w') as hf:
        for k, v in data_objects_dict.items():
            try:
                ...
                hf.create_dataset(k, data=v, chunks=True, compression="gzip")  
                print(f"✅ Saved {k} in HDF5 file")
            except Exception as e:
                failed.append(k)
                print(f"❌ Failed to save {k} as HDF5. Error: {e}. Will try Pickle.")
