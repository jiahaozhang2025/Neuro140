...
def ordering_split(voxel, ordering, combine_trial=False):
    ...
    if combine_trial:        
        ...
        val_voxel_data = voxel[shared_mask].copy()
        val_stim_ordering  = ordering_data[shared_mask].copy()

        trn_voxel_data = voxel[~shared_mask].copy()
        trn_stim_ordering  = ordering_data[~shared_mask].copy()
